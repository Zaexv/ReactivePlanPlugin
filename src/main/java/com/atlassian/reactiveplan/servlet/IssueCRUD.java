...
public class IssueCRUD extends HttpServlet{
    ...
    private List<Issue> getIssues() {
        ...
        Query query = jqlClauseBuilder.project("PDP").buildQuery();
    ...
    private void handleIssueCreation(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        Project project = projectService.getProjectByKey(user, "PDP").getProject();
        ...
        IssueType taskIssueType = constantsManager.getAllIssueTypeObjects().stream().filter( //los IssueTypes si tienes JIRA configurado en Español están en Español.
                issueType -> issueType.getName().equalsIgnoreCase("tarea")).findFirst().get();
    ...
    private void doGetOLD(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
    {
        ...
        resp.getWriter().write("<html><body>Hello World"
                + ComponentAccessor.getJiraAuthenticationContext().getLoggedInUser().toString() +
                " is the logged user. " +
                "" +
                "Hakuna matata, ningún problema debe hacerte sufrir.</body></html>");

        //Devuelve el usuario loggeado en la aplicación.
